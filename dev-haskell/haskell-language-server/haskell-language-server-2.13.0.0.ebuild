...
RDEPEND="
	...
	~dev-haskell/ghcide-2.13.0.0:=[profile?]
	...
	hls_plugins_hlint? (
		...
		dev-haskell/unordered-containers:=[profile?]
		ghc-lib? ( >=dev-haskell/ghc-lib-parser-9.8:=[profile?] <dev-haskell/ghc-lib-parser-9.15:=[profile?] )
		!ghc-lib? ( >=dev-lang/ghc-9.8:= <dev-lang/ghc-9.15:= )
	)
	hls_plugins_import-lens? (
		dev-haskell/aeson:=[profile?]
		~dev-haskell/hls-graph-2.13.0.0:=[profile?]
	...
	hls_plugins_ormolu? (
		>=dev-haskell/ormolu-0.1.2:=[profile?] <dev-haskell/ormolu-0.9:=[profile?]
...
src_configure() {
	config_flags=(
		$(cabal_flag ghcide-bench ghcide-bench)
		$(cabal_flag ghcide-bench-lib ghcide-bench-lib)
		...
		$(cabal_flag hls_plugins_ormolu ormolu)
		...
		--flag=$(usex ghcide-test-exe "test-exe" "-test-exe")
		--flag=-fourmolu
		--flag=-dynamic
		--flag=-ghc-lib
		--flag=-pedantic
